...
@section('content')
    {{-- Memuat SweetAlert2 (CDN) untuk Notifikasi Cantik --}}
        ...
        <div class="container-fluid px-lg-4 px-xl-5">
            ...
            <div class="row align-items-center mb-5">
                ...
                <div class="col-md-5 mt-3 mt-md-0 d-flex justify-content-md-end gap-3">
                    <div class="card border-0 shadow-sm px-3 py-2 text-center" style="min-width: 100px;">
                        ...
                        <span class="h4 fw-bold text-primary mb-0">{{ $myTasks->where('pivot.is_completed', false)->count() }}</span>
                    </div>
                    <div class="card border-0 shadow-sm px-3 py-2 text-center" style="min-width: 100px;">
                        <small class="text-muted d-block fw-bold" style="font-size: 0.7rem;">SELESAI</small>
                        <span class="h4 fw-bold text-success mb-0">{{ $myTasks->where('pivot.is_completed', true)->count() }}</span>
                    </div>
                </div>
            </div>
            ...
            <ul class="nav nav-pills mb-4 gap-2" id="pills-tab" role="tablist">
                ...
                <li class="nav-item ms-auto" role="presentation">
                    <button class="nav-link btn-outline-dashed px-4" onclick="document.getElementById('available-tasks').scrollIntoView({behavior: 'smooth'})">
                        ...
                        <div class="row g-4">
                            @foreach($ongoingTasks as $task)
                                ...
                                <div class="col-xl-4 col-lg-6">
                                    <div class="card border-0 shadow-sm h-100 task-card hover-lift {{ $borderClass }}" style="border-left: 5px solid !important;">
                                        <div class="card-body p-4 d-flex flex-column">

                                            {{-- Header Kartu --}}
                                            <div class="d-flex justify-content-between align-items-start mb-3">
                                                <span class="badge bg-{{ $statusColor }} bg-opacity-10 text-{{ $statusColor }} py-2 px-3 rounded-pill fw-bold">
                                            ...
                                            <div class="d-flex gap-2 mt-auto">
                                                <form action="{{ route('my-tasks.update', $task->id) }}" method="POST" class="flex-grow-1 confirm-finish-form">
                                                    @csrf
                                                    @method('PATCH')
                                                ...
                                                <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#detailModal{{ $task->id }}">
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <div class="row mt-4">
                                                    ...
                                                    <div class="col-6 text-end">
                                                        ...
                                                        <span class="badge bg-{{ $statusColor }}">{{ $deadline->diffForHumans() }}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                ...
                <div class="tab-pane fade" id="pills-history" role="tabpanel">
                    ...
                    <div class="card border-0 shadow-sm overflow-hidden">
                        <div class="card-header bg-white py-3">
                            <h5 class="mb-0 fw-bold text-success"><i class="fas fa-check-double me-2"></i> Tugas Telah Diselesaikan</h5>
                        </div>
                                        ...
                                        <tbody>
                                            @foreach($completedTasks as $task)
                                                <tr>
                                                    ...
                                                    <td class="text-muted small">{{ \Carbon\Carbon::parse($task->deadline)->format('d M Y') }}</td>
                                                    ...
                                                    <td class="text-center">
                                                        ...
                                                        <form action="{{ route('my-tasks.destroy', $task->id) }}" method="POST" class="d-inline delete-history-form">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="button" class="btn btn-sm btn-outline-danger btn-delete-history" title="Hapus dari Riwayat">
                                                                <i class="fas fa-trash-alt"></i>
                                                            </button>
                                                        </form>
                                                        {{-- Tombol Batal Selesai (Opsional) --}}
                                                        <form action="{{ route('my-tasks.update', $task->id) }}" method="POST" class="d-inline ms-1">
                                                            @csrf
                                                            @method('PATCH')
                                                            <button type="submit" class="btn btn-sm btn-outline-secondary" title="Kembalikan ke Aktif">
                                        ...
                                        <tbody>
                                            @foreach($availableTasks as $task)
                                                <tr class="hover-lift-row">
                                                    ...
                                                    <td class="text-muted text-truncate" style="max-width: 250px;">{{ Str::limit($task->deskripsi, 60) }}</td>
                                                    <td>
                                                        <span class="badge bg-light text-dark border">
                                                            <i class="far fa-calendar me-1"></i> {{ date('d M Y', strtotime($task->deadline)) }}
                                                        </span>
                                                    </td>
                                                    <td class="text-center">
                                                        <form action="{{ route('my-tasks.store') }}" method="POST" class="take-task-form">
                                                            ...
                                                            <button type="button" class="btn btn-sm btn-primary px-3 rounded-pill btn-take-task">
        ...
        <style>
            ...
            .border-warning { border-color: #fca5a5 !important; } /* Sedikit merah muda untuk warning agar beda */
